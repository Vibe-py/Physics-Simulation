#include <cstdio>
#include <unordered_map>
#include <chrono>
#include <vector>
#include <math.h>

#include "Headers\Fabric\Engine\WindowContextHandler.h"
#include "Headers\Fabric\Engine\Physics\Physics.h"
#include "Headers\Fabric\Renderer.h"
#include "Headers\Fabric\Wind.h"
#include "Headers\Fabric\Utils.h"
#include "Headers\Globals.h"
#include "Headers\Mass.h"
#include "Headers\Free Falling\FreeFalling.h"
#include "Headers\Window.h"
#include "Headers\Double Pendulum\DoublePendulum.h"

int main()
{
    sf::ContextSettings settings;
    //settings.antialiasingLevel = 4;

    FreeFalling appleObject("imgs/apple.png", Pos(0, 0));

    std::chrono::microseconds lag(0);
    std::chrono::steady_clock::time_point previousTime;

    previousTime = std::chrono::steady_clock::now();

    float clickRadius = 10.0f;

    const uint32_t window_width = 1920;
    const uint32_t window_height = 1080;
    WindowContextHandler app("Physics Simulation", sf::Vector2u(window_width, window_height), sf::Style::Default);
    
    PhysicsSolver solver;
    Renderer renderer(solver);

    Pendulum topPendulum(100, 90, 220, app.getWindowSize().x / 2, 175);
    Pendulum lowerPendulum(100, 90, 220, &topPendulum);
    
    const uint32_t cloth_width = 150;
    const uint32_t cloth_height = 100;
    const float    links_length = 20.0f;
    const float    start_x = (window_width - (cloth_width - 1) * links_length) * 0.5f + cloth_width * 15;
    for (uint32_t y(0); y < cloth_height; ++y) {
        const float max_elongation = 1.2f * (2.0f - to<float>(y) / float(cloth_height));
        for (uint32_t x(0); x < cloth_width; ++x) {
            const civ::ID id = solver.addParticle(
                sf::Vector2f(start_x + to<float>(x) * links_length, to<float>(y) * links_length)
            );
            if (x > 0) {
                solver.addLink(id - 1, id, max_elongation * 0.9f);
            }
            if (y > 0) {
                solver.addLink(id - cloth_width, id, max_elongation);
            }
            else {
                solver.objects[id].moving = false;
            }
        }
    }


    sf::Vector2f last_mouse_position;
    bool dragging = false;
    bool erasing = false;
    app.getEventManager().addMousePressedCallback(sf::Mouse::Right, [&](sfev::CstEv) {
        dragging = true;
        last_mouse_position = app.getWorldMousePosition();
        });
    app.getEventManager().addMouseReleasedCallback(sf::Mouse::Right, [&](sfev::CstEv) {
        dragging = false;
        });
    app.getEventManager().addMousePressedCallback(sf::Mouse::Middle, [&](sfev::CstEv) {
        erasing = true;
        });
    app.getEventManager().addMouseReleasedCallback(sf::Mouse::Middle, [&](sfev::CstEv) {
        erasing = false;
        });
    app.getEventManager().addKeyPressedCallback(sf::Keyboard::Equal, [&](sfev::CstEv) {
        if (clickRadius >= 50.0f) return;
        clickRadius += 1;
        });
    app.getEventManager().addKeyPressedCallback(sf::Keyboard::Hyphen, [&](sfev::CstEv) {
        if (clickRadius <= 10.0f) return;
        clickRadius -= 1;
        });
    app.getEventManager().addKeyPressedCallback(sf::Keyboard::C, [&](sfev::CstEv) {
        renderer.ChangeColor();
        });
    app.getEventManager().addKeyPressedCallback(sf::Keyboard::R, [&](sfev::CstEv) {
        appleObject.setPos(0, 0);
        solver.objects.clear();
        topPendulum.init(100, 90, 220);
        lowerPendulum.init(100, 90, 220);
        for (uint32_t y(0); y < cloth_height; ++y) {
            const float max_elongation = 1.2f * (2.0f - to<float>(y) / float(cloth_height));
            for (uint32_t x(0); x < cloth_width; ++x) {
                const civ::ID id = solver.addParticle(
                    sf::Vector2f(start_x + to<float>(x) * links_length, to<float>(y) * links_length)
                );
                if (x > 0) {
                    solver.addLink(id - 1, id, max_elongation * 0.9f);
                }
                if (y > 0) {
                    solver.addLink(id - cloth_width, id, max_elongation);
                }
                else {
                    solver.objects[id].moving = false;
                }
            }
        }
        });

    sf::Clock clock;


    WindManager wind(to<float>(window_width));
    wind.winds.emplace_back(
        sf::Vector2f(200.0f, window_height + cloth_width),
        sf::Vector2f(0.0f, 0.0f),
        sf::Vector2f(2000.0f, 0.0f)
    );
    wind.winds.emplace_back(
        sf::Vector2f(40.0f, window_height + cloth_height),
        sf::Vector2f(0.0f, 0.0f),
        sf::Vector2f(6000.0f, 0.0f)
    );

    const float dt = 1.0f / 60.0f;
    while (app.run()) {
        const sf::Vector2f mouse_position = app.getWorldMousePosition();

        if (dragging) {
            const sf::Vector2f mouse_speed = mouse_position - last_mouse_position;
            last_mouse_position = mouse_position;
            usr::Utils::applyForceOnCloth(mouse_position, 100.0f, mouse_speed * 8000.0f, solver);
        }

        if (erasing) {
            solver.objects.remove_if([&](const Particle& p) {return usr::Utils::isInRadius(p, mouse_position, clickRadius); });
        }
        // Update physics ;)
        wind.update(solver, dt);
        solver.update(dt);

        appleObject.Fall();

        lowerPendulum.update();
        lowerPendulum.updatePos(clock.getElapsedTime().asSeconds() * 20);
        topPendulum.updatePos(clock.getElapsedTime().asSeconds() * 20);

        clock.restart();

        // Render POGGERS SHEEESH YOOOOOOOOOO LOL XD IT WORKS KJGFSDLKFHJA
        RenderContext& render_context = app.getRenderContext();
        render_context.clear(sf::Color::White);
        renderer.render(render_context);
        if (cFrameDur > lag)
        {
            // pendulums
            render_context.draw(appleObject.getSprite());
            render_context.draw(topPendulum.getArm());
            render_context.draw(lowerPendulum.getArm());
            render_context.draw(topPendulum.getBody());
            render_context.draw(lowerPendulum.getBody());
        }
        render_context.display();
    }

    return 0;
}

